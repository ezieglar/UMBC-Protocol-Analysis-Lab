(herald "Session Binding Protocol with TLS 1.2 (proxy and server separate as in paper description (TLS connection from proxy to server), server isolated on a private network with a confidential connection to the proxy)."
	(bound 12)
	)
...
(defprotocol ca basic
  ...
  (defrole prxyr ;; Role where proxy takes requests within a TLS session
...
(defskeleton ca
  ...
  (uniq-orig cr pms)
  (uniq-orig sr) ;; This is only included to prevent multiple runs of the same
  ;; server interacting with the same client. (It can be removed and it will
  ;; not impact the security properties of the protocol.)
  )
